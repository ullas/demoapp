    		...
    		if ($userrole=="admin" || $userrole=="employee") {
    			if( (isset($counts['legalentity']) && $counts['legalentity'] >0) && (isset($counts['businessunit']) && $counts['businessunit'] >0) &&
					...
					(isset($counts['costcenter']) && $counts['costcenter'] >0) && (isset($counts['position']) && $counts['position'] >0) ){
						echo "<p><a style='display: inline-block;white-space: nowrap;overflow: hidden; text-overflow: ellipsis;-o-text-overflow: ellipsis;max-width: 90%;' href='/Employees/view/".$this->request->session()->read('sessionuser')["employee_id"]."'>".$empname."</a></p>";
				}else{
					echo "<p>".$empname."</p>";
				}
			}else{
