...
//
	...
	function validate(empid){
		...
		if($("#period").val()=="" || $("#period").val()==null){
			sweet_alert("Please enter the payroll period for the employee ."+empid);
            return false;
		}
		...
		$(".payrollprogresstitle").html("Validating");
    		...
    		$.ajax({
        			...
        			if(data=="success"){
        				...
        				$.ajax({
        					...
        					error : function(result) {
            					...
            					sweet_alert("No Pay Component/Group existing for the employee ."+empid);
            					return false;
        					}
    					});
        		...
        		error : function(data) {$("#"+empid+".statustxt").html("Leave approval validation failed");
            		sweet_alert("Pending leave approvals for employee ."+empid);
            		return false;
        		}
    		});
	}
	function pushpayrollstatus(empid){
				var period=$("#period").val();
				...
				$.ajax({
        			...
        			data: 'empid='+empid+"&currentperiod="+period,
        			success : function(data) {
            			return false;
    				},
        			error : function(data) {
	...
	function setfilter(){
	   ...
	   paygroupflagActive  ? $(".preprocessselected").show() : 	$(".preprocessselected").hide();
	}
		</script>
 <?php $this->end(); ?>
