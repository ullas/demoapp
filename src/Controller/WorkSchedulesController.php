		...
		if(isset($data)){
		   foreach($data as $key =>$value){
			    ...
			    if(count($itemna)== 2 && $itemna[0]=='chk'){
					...
					$record = $this->WorkSchedules->get($value);
					
					 if($record['customer_id']== $this->loggedinuser['customer_id']) {
					 	
						   if ($this->WorkSchedules->delete($record)) {
					           $sucess= $sucess | true;
					        } else {
				...
				if($sucess){
					$this->Flash->success(__('Selected WorkSchedules has been deleted.'));
				}
		        if($failure){
					$this->Flash->error(__('The WorkSchedules could not be deleted. Please, try again.'));
				}
