		...
		if(isset($data)){
		   foreach($data as $key =>$value){
			    ...
			    if(count($itemna)== 2 && $itemna[0]=='chk'){
					...
					$record = $this->JobFunctions->get($value);
					
					 if($record['customer_id']== $this->loggedinuser['customer_id']) {
					 	
						   if ($this->JobFunctions->delete($record)) {
					           $sucess= $sucess | true;
					        } else {
				...
				if($sucess){
					$this->Flash->success(__('Selected JobFunctions has been deleted.'));
				}
		        if($failure){
					$this->Flash->error(__('The JobFunctions could not be deleted. Please, try again.'));
				}
