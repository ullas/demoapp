		...
		if(isset($data)){
		   foreach($data as $key =>$value){
			    ...
			    if(count($itemna)== 2 && $itemna[0]=='chk'){
					...
					$record = $this->CalendarAssignments->get($value);
					
					 if($record['customer_id']== $this->loggedinuser['customer_id']) {
					 	
						   if ($this->CalendarAssignments->delete($record)) {
					           $sucess= $sucess | true;
					        } else {
				...
				if($sucess){
					$this->Flash->success(__('Selected CalendarAssignments has been deleted.'));
				}
		        if($failure){
					$this->Flash->error(__('The CalendarAssignments could not be deleted. Please, try again.'));
				}
